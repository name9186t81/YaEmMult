...
namespace Networking
{
	public sealed class DebugMessageSender : MonoBehaviour
	{
		...
		private void Start()
		{
			_sendToServer.onClick.AddListener(() => { ServiceLocator.Get<ListenersCombiner>().Client.SendPackage(new ChatMessagePackage(_input.text), ServiceLocator.Get<ListenersCombiner>().Server.SelfEndPoint); Debug.LogWarning(ServiceLocator.Get<ListenersCombiner>().Server.SelfEndPoint); });
